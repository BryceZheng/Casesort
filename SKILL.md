...
**方式 C：PaddleOCR**
```bash
python3 ~/.claude/skills/paddleocr-ocr/paddle_ocr.py \
  "<用户提供的PDF路径>" \
  "<用户提供的PDF路径>/Markdown_转换结果" \
  --token "YOUR_PADDLEOCR_API_TOKEN"
```
...
```bash
# 第一步：将 JSON 写入临时文件
# 使用 Write 工具写到 scratchpad 目录，如：
# 例：/tmp/casesort_phase3_batchN.json

# 第二步：传入脚本
python3 ~/.claude/skills/Casesort/phase3_excel.py \
...
选择格式 B 时，Claude 根据案件类型推荐额外字段，告知用户后确认：
...
```bash
# 第一步：用 Write 工具将 JSON 写入 scratchpad 临时文件
# 第二步：传入脚本
python3 ~/.claude/skills/Casesort/phase4_excel.py \
  --format A \
...
用户确认后，按以下结构撰写报告，并调用 `docx` skill 生成 Word 文档：
...
每类包含：
- 该类案例的典型共性（违法手段、当事人特征、法律适用等）
- 潜在法律风险分析
- **查询表格**（每类必须附表，位于该类段落末尾）：
...
**步骤 4：调用 docx skill 输出 Word**

调用 `docx` skill，生成格式规范的 Word 文档：
- 标题层级分明（一级/二级/三级标题）
- 语言专业严谨
- 表格格式规范
- 输出路径：`<案例库Excel同级目录>/案例分析报告.docx`
